...
本指南适用于 `seo-report-portal-v2-5`。每位同事都能完成**本地 → SMB → OSS**，但不得跳过数据、付费或发布批准门禁。
...
source_dir="$HOME/.codex/sources/seo-report-portal-v2-5"
skill_dir="$HOME/.codex/skills/seo-report-portal-v2-5"
backup_dir="$HOME/.codex/skill-backups/seo-report-portal-v2-5-pre-update-$(date +%Y%m%d%H%M%S)"
...
$source = Join-Path $env:USERPROFILE '.codex\sources\seo-report-portal-v2-5'
$archive = Join-Path $env:TEMP ('seo-report-portal-v2-5-' + [guid]::NewGuid() + '.zip')
$stage = Join-Path $env:TEMP ('seo-report-portal-v2-5-stage-' + [guid]::NewGuid())
$skill = Join-Path $env:USERPROFILE '.codex\skills\seo-report-portal-v2-5'
$backup = Join-Path $env:USERPROFILE ('.codex\skill-backups\seo-report-portal-v2-5-pre-update-' + (Get-Date -Format yyyyMMddHHmmss))
...
替换后重启 Codex，让新 Skill 生效；不需要重建客户工作区或重新填写任何密钥。出现问题时，将新目录移走，再把对应时间戳备份目录移回 `~/.codex/skills/seo-report-portal-v2-5`（Windows 使用同样的 `Move-Item`），然后重新启动 Codex。上述命令只删除临时 Git archive；不会删除任何旧 Skill、客户工作区或私密配置。

Windows 同事如需将“更新 v2.5、验证 UNC、迁移当前工作区已有 v2.2 原始档案”交由 Codex 一次完成，可将 `references/windows-v24-repair-runbook.md` 作为任务附件交给 Codex。该运行手册不扫描整个磁盘、不自动重采集 Google 数据，也不会触碰 `private/` 或 `~/.codex/config.toml`。
...
3. 每次采集必须传入 `--archive-root <共享原始档案根目录> --month YYYY-MM`；每次生成也必须传入同一个 `--archive-root`。采集/导入完成前会先独占写入 JSON，再写同名非公开 `.json.ready` SHA-256 标记；只有两者匹配才可生成、发布或自动选词。缺完整上期只会使对比不可用；当前报告期缺档案仍会阻止生成。非标准连续跨度使用 `--type period`，不能标为季度报告。
...
```powershell
py -3 "$skill\scripts\publish_oss_report.py" --local-report-dir <output/dashboards/domain/type/period> --client-slug <client-slug> --type <monthly|quarterly|yearly|period> --period <period> --source-archive-root <shared-source-root> --dry-run
```

## 每份报告的发布步骤

1. 采集并归档 GA4/GSC 官方月度数据，绝不改写历史档案。
2. 先复用同客户、同完整 `reporting_period`、范围一致且带 `approval.approved: true` 的 DataForSEO 和 SEOAgent 归档。任一缺失或范围变化时，先给出该次请求范围、费用上限并获得明确确认；用户明确不用时才在生成命令中加 `--without-third-party`。
